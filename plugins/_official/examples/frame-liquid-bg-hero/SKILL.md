...
[Intent] Works as a video intro frame, a SaaS landing-page top hero, or a poster background. WebGL-fluid feel, but degrades gracefully to CSS / canvas drawing so the single file still opens by double-click. Inspired by hyperframes vfx-liquid-background.

[Canvas] 1920×1080 (landscape) or 1080×1920 (portrait), pick one. Background fills the frame.

[Fluid background — 3 implementations, choose per user preference]
1. **CSS multi-layer radial-gradient offset breathing** (most stable, default recommendation):
   - 3-5 large ellipses via `radial-gradient(...)`, colors drawn from the palette.
   - Each ellipse gets `@keyframes` translate + scale + hue-rotate, 8-14s cycle, staggered; the whole frame overlaid with `mix-blend-mode: screen` or `overlay`.
   - Top layer adds a `backdrop-filter: blur(80px)` for softer edges.
...
3. **WebGL fragment shader** (advanced, use sparingly):
   - Load `regl` via jsdelivr CDN, or write inline plain WebGL.
   - Shader implements domain-warp noise; a single quad, one `u_time` uniform.

[Top text layer]
- Centered or bottom-left: one giant quote (5-7vw, serif or bold sans), font: `Source Serif Pro` / `Inter Tight` / `Manrope Black`.
- Text color paper white `#fafaf8` or ink, depending on background lightness; add `mix-blend-mode: difference` so it stays readable over any fluid color.
- Subtitle (small sans, opacity 0.7) one line.
- Optional bottom CTA chip or a hairline + metadata row.

[Palette — choose 1 of 4, no rainbow]
...
- Fonts: for CJK use `Noto Serif SC` (display) / `Noto Sans SC` (subtitle).
- No external image links; everything is CSS + SVG + optional canvas.
- Must use the user's provided quote / title; if the user's input is data -> distill it into a quote of ≤18 words.
