...
description: |
  ...
  reflective flowing surface. MishMash serves this in powered-preview mode
...
MishMash detects `getContext('webgl2')` and renders this file in **powered preview** — a cross-origin-isolated iframe with `allow-same-origin`. The full GPU pipeline is available; you do not need to work around the opaque sandbox.
