...
example_tagline: "Spotify's classic dark card"
...
[Intent] Render a song, a podcast episode, or a personal bio as a Spotify Now Playing card, suited to video overlays / personal about pages / creator heroes. Inspired by hyperframes spotify-card.
...
[Card structure]
- Outer frame: 12-16px corner radius; bg is a dark gradient sampled from the album art (e.g. `linear-gradient(135deg, #1e3264 0%, #0d1f3d 100%)`) or Spotify's classic `#121212`; 1px subtle border at the edge.
- Left side: **album art** (CSS gradient + large monogram type or abstract geometric drawing, no external image links), 6px corner radius, 60-200px square.
- Right side:
...
- Top-right: Spotify logo (inline SVG, green `#1DB954` circle + three white sound waves).
- Optional: a small waveform animation bottom-right (3 bars via `@keyframes`).

[Fonts]
- Primary: `Spotify Circular` -> fallback `Inter` / `Inter Tight`, weight 400 / 700.
- Numbers: same as the primary font, avoid excessive mono.

[Design details]
- Spotify's classic dark mode: `#121212` bg, `#1DB954` accent, `#b3b3b3` secondary text.
- If the user's input is text/a title -> treat "the title" as the track name, "subtitle/author" as the artist, default "duration" estimate to 3:42.
- If the user's input is music-related -> map it directly.
- No external image links; render the cover art with a CSS gradient + text logo / geometric drawing.
- Micro-motion: the waveform animation uses `@keyframes`, can be disabled via `prefers-reduced-motion`.
- Single-file HTML.
