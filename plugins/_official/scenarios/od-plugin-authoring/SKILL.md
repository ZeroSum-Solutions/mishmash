...
description: Guided scenario for creating a MishMash plugin folder that can be installed into My plugins.
od:
...
Use this scenario when the user wants to create their own MishMash plugin.
...
- Treat `SKILL.md` as the canonical behavior description. `open-design.json` should describe how MishMash installs, applies, and presents that behavior.
...
- Include a short readiness summary when finished:
  ...
  - A direct next-action prompt that offers: Add to My plugins, Publish repo, or MishMash PR.
