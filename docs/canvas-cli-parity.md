...
| draw-mark-annotation | editing | UI-ONLY | `od export --format image --width <px> --height <px> --scroll-y <px>` (the capture half only) | The capture half is now CLI-reachable: annotation capture asks the daemon for a viewport-clipped render (CANVAS-13), and the same request is drivable from `od export`. The rest is not: structured comment attachment still goes through `apps/web/src/comments.ts` and the composer's upload path (`uploadProjectFiles` → `POST /api/projects/:id/upload`, see composer-file-picker below), and neither has a CLI verb. |
...
- **draw-mark-annotation** — the capture half landed with CANVAS-13 (`od export --format image --scroll-y`). What remains depends on both the comments endpoint above and the composer upload endpoint below; `od draw send --project <id> --screenshot <path> --bounds <json>` once both exist.
