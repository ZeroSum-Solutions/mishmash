...
|---|---|---|
| unit | `pnpm --filter @open-design/daemon test` (`vitest run -c vitest.config.ts`, 495 test files, 6157 assertions) | **unit: 0 failures** |
| integration | `pnpm --filter e2e exec vitest run -c vitest.config.ts tests` (26 files, cross-app/cross-runtime consistency checks under `e2e/tests/`, 137 assertions) | **integration: 0 failures** |
...
- **`apps/daemon/tests/project-upload-filenames.test.ts`** — passed in this
  run (part of the 6157). No reproduction captured in this session.
- **`e2e/tests/amr/logout-state-persistence.test.ts`** (ENOTEMPTY cleanup
  race) — passed in this run (part of the integration layer's 137). The
