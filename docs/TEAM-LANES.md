...
- **How to use it:** attach the skill to the run (skill/template picker, or
  ask for it by name in the brief). Skills are not auto-discovered by the
  inner agent — if you don't attach it, the agent may not know it exists.
  (Exception: the Claude lane on this machine also has a global Higgsfield
  skill installed, so it can usually generate images even without the
  attachment.)
...
- Kimi CLI config: `kimi provider catalog add moonshotai --default-model
  ...
  time (key lives in ZS Vault as `moonshot_api_key`). Verify with
  `kimi provider list` and a `kimi -p` smoke test.
- Server-side model pin: set `KIMI_DEFAULT_MODEL=moonshotai/kimi-k3` in the
  daemon's environment (launchd plist on the mini) to pin the default
  without touching per-install app config.
