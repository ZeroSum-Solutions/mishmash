...
| `OD_*` env vars | `/\bOD_[A-Z_]+\b/g` | 2683 | Every `OD_DATA_DIR`, `OD_BIND_HOST`, `OD_SANDBOX_MODE`, `OD_LEGACY_DATA_DIR`, etc. reference across daemon, web, tools, docs, and tests. |
...
- **`.od/` and `OD_*`** are the two categories any rename would actually
  ...
  every `OD_*` env var name. Both are large (109 + 2683 sites) precisely
