...
| `OD_*` env vars | `/\bOD_[A-Z_]+\b/g` | 2718 | Every `OD_DATA_DIR`, `OD_BIND_HOST`, `OD_SANDBOX_MODE`, `OD_LEGACY_DATA_DIR`, etc. reference across daemon, web, tools, docs, and tests. Recounted after the reviewed TMPDIR-robustness test fixes, the capability-manifest reason documentation, and the CLI stdout-drain red-spec test (whose daemon-boot env setup necessarily references these vars) added sites (note kept digit-free: the count column's number must stay the row's last numeral for the verifier's parser). |
