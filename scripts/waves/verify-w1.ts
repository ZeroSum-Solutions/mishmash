...
// FOUNDER-AUTHORIZED OUT-OF-BAND REPLY REDESIGN (round 4, 2026-07-28): round
// 3's FAKE_REPLY_MARKER (a fixed-per-invocation CSPRNG prefix embedded in
// BOTH the init event's `model` field and the reply text) was ruled still
// exploitable without ever knowing the marker's value: "the fake emits the
// executed model first in the init event (:1027), then emits <marker>
// <executedModel> (:1028). Product code can therefore use one stable
// predicate -- content.endsWith(' ' + reportedModel) -- and populate
// routing truth... without knowing the marker." The reply text is now a
// PURE RUNTIME TOKEN with NO relationship whatsoever to the init event's
// model field or to any verifier-process constant -- see FAKE_CLAUDE_SCRIPT
// below, where the token is generated by crypto INSIDE the spawned script
// at reply time, never baked in from this process. The executed model is
// persisted out-of-band, keyed by that token, in FAKE_CLAUDE_RESULTS_DIR
// (mkdtempSync'd once here, at module scope, exactly like
// FAKE_CLAUDE_SCRIPT itself -- FAKE_CLAUDE_SCRIPT is a single shared
// constant reused across every criterion that spawns a fake claude, not a
// per-criterion value, so the results directory it embeds must also be
// module-scoped: a per-checkCriterion directory would require touching the
// writeFakeBin call site inside all twelve-plus consumers, including the
// four protected bodies, which round 3's authorization does not permit).
// The directory's path is baked into the generated script's own source at
// module-load time, the same trust profile already accepted for
// buildFakeKimiScript's specDir: reading a colocated fake CLI's own source
// off disk to recover this path is a deliberate, multi-step action, not the
// single fixed predicate the confirmation flagged.
const FAKE_CLAUDE_RESULTS_DIR = fs.mkdtempSync(path.join(os.tmpdir(), randomNonce(8)));
// Consumer-side lookup: validates the persisted reply text is plausibly one
// of this script's own tokens (crypto.randomBytes(16).toString('hex') is
// always exactly 32 lowercase hex characters -- a shape check, not a value
// check; it rejects garbage without ever encoding meaning INTO the token
// itself), reads the matching result file, and returns a
// RegExpExecArray-compatible `[fullText, executedModel]` tuple so every
// existing `m?.[1]` / `executedMatch?.[1]` call site keeps working
// unchanged. Any failure (bad shape, missing file, unparsable JSON, wrong
// field type) returns null, which already flows into each consumer's
// existing "could not independently observe the executed model" failure
// path -- no consumer-side error handling needed beyond the swap itself.
function lookupFakeClaudeReply(replyText: string): [string, string] | null {
  if (!/^[0-9a-f]{32}$/.test(replyText)) return null;
  try {
    const raw = fs.readFileSync(path.join(FAKE_CLAUDE_RESULTS_DIR, `${replyText}.json`), 'utf8');
    const parsed: unknown = JSON.parse(raw);
    if (!isRecord(parsed) || typeof parsed.executedModel !== 'string') return null;
    return [replyText, parsed.executedModel];
  } catch {
    return null;
  }
}
...
function respond(promptText) {
  ...
  // FOUNDER-AUTHORIZED OUT-OF-BAND REPLY REDESIGN (round 4, fixture
  // literals -- 3 of 3, final form): round 3's marker-prefixed reply
  // ("<marker> <executedModel>") was ruled still exploitable WITHOUT ever
  // knowing the marker, because the SAME executedModel string is also the
  // init event's own \`model\` field a few lines above -- "content.endsWith('
  // ' + reportedModel)" is a stable predicate needing no marker at all. The
  // reply is now a PURE RUNTIME TOKEN generated fresh right here, inside
  // this spawned process, from ITS OWN crypto -- structurally unrelated to
  // \`executedModel\` and to any value this verifier's own process embedded
  // in this script's source. The real executedModel is persisted
  // out-of-band, keyed by that token, for the verifier's own
  // lookupFakeClaudeReply helper to resolve post-hoc (see its comment in
  // verify-w1.ts, above this script's own generation).
  const sessionId = crypto.randomBytes(8).toString('hex');
  const assistantMsgId = crypto.randomBytes(8).toString('hex');
  const replyToken = crypto.randomBytes(16).toString('hex');
  const RESULTS_DIR = ${JSON.stringify(FAKE_CLAUDE_RESULTS_DIR)};
  fs.writeFileSync(path.join(RESULTS_DIR, replyToken + '.json'), JSON.stringify({ executedModel }));
  ...
  line({ type: 'assistant', message: { id: assistantMsgId, role: 'assistant', content: [{ type: 'text', text: replyToken }], stop_reason: 'end_turn' } });
...
await checkCriterion('C1-1', 'contract type scan (packages/contracts/src/**) + two real HTTP runs via a verifier-owned fake claude binary, ground-truthed against the daemon\'s own sqlite messages table AND the fake CLI\'s own independent echo', 'a known model persists requested===resolved===reported; an unresolvable model forces resolved!=requested, resolved matches the independently-observed executed model, and reported is non-null and consistent', async () => {
  ...
  try {
    ...
    function executedModelFromContent(msg: MessageRow | null): string | null {
      const m = lookupFakeClaudeReply(msg?.content ?? '');
      return m?.[1] ?? null;
    }
...
await checkCriterion('C1-2', 'apps/web/src/components/agentModelSelection.ts direct call (non-amr agent, unknown model id) + tools-dev/Playwright real DOM check', 'a disabled/unknown model id is corrected for agents other than amr, and the substitution renders as real, accessible text naming both models', async () => {
  ...
  if (!pw.ok) {
    ...
    try {
      ...
      const executedMatch = lookupFakeClaudeReply(msg?.content ?? '');
...
await checkCriterion('C1-5', 'local PostHog-shaped capture stub + a substitution-triggering run (unresolvable model id, non-amr agent)', 'the model_id property stamped on properties.model_id of the captured analytics event equals the resolved model, not the raw requested one', async () => {
  ...
  try {
    ...
    const executedMatch = lookupFakeClaudeReply(msgForExec?.content ?? '');
    const executedModel = executedMatch?.[1] ?? null;
...
await checkCriterion('C1-11', 'CDP Accessibility-domain computed audit across THREE states: a named descendant control inside the substituted message, a named descendant control inside the unverified (codex, no-echo) message, and the model picker reached from / per the e2e house pattern', 'the substituted message has a descendant control whose computed accessible name contains BOTH the requested and resolved model; the unverified message has one naming "unverified"; the picker is present, visible, and has a non-empty computed accessible name with no generic/none/presentation role', async () => {
  ...
  try {
    ...
    const executedMatch = lookupFakeClaudeReply(substitutedMsg?.content ?? '');
    const executedModel = executedMatch?.[1] ?? null;
