...
test("accepts directory-layout and legacy flat-layout entries", () => {
  const manifest = manifestWith({
    ...
    "sample-invoice": flatEntry("sample-invoice", HASH_B),
  });
  const ids = new Set(["example-video-hyperframes", "sample-invoice"]);
...
test("flags an orphan entry whose plugin no longer ships", () => {
  ...
  const violations = validatePreviewManifest(manifest, new Set(["sample-invoice"]));
...
test("flags a key whose embedded fingerprint disagrees with the recorded hash", () => {
  const manifest = manifestWith({
    "sample-invoice": { ...flatEntry("sample-invoice", HASH_A), hash: HASH_B },
  });
  const violations = validatePreviewManifest(manifest, new Set(["sample-invoice"]));
