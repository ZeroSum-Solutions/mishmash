...
- **Auto-checked.** Rules wired into `apps/daemon/src/lint-artifact.ts` — the P0 list in `anti-ai-slop.md` (Tailwind-indigo accent, two-stop hero gradients, emoji-as-icons, etc.) plus one P1 rule from `composition.md` (`layout-risk-flat`: a long, multi-section page with zero source-level evidence of any grid-breaking move). `apps/daemon/src/server.ts`'s Claude run handler lints every `.html`/`.htm` file the run itself writes (not one it only reads or copies), once per distinct content, and on a P0 finding writes the findings back onto the agent's own turn as a follow-up message — self-correction the agent can act on before the turn ends. This channel is Claude-only: it rides the same stdin-stays-open mechanism `apps/daemon/src/runtimes/chat-run-lifecycle.ts` uses for mid-turn input, which no other wired runtime has, so a run on another agent gets no in-turn lint feedback. A web-clone run (`isWebCloneRun`) gets the same `layout-risk-flat` exemption `craft.ts` already gives it for `craft/composition.md`. There is still no UI surface for these findings — no P0/P1 badge — so today the loop is agent-only, not user-visible; `POST /api/artifacts/save` and `POST /api/artifacts/lint` also call the linter but have no caller anywhere in this codebase, and are not part of the enforced path. Artifact persistence is not currently hard-blocked on P0 (or P1) hits.
...
(© Refero Design), with edits to fit MishMash's house style and link
