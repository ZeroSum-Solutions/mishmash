// Baseline request-rate limit for GET /api/library/connection, closing the
// ground-facts gap "no request- or byte-volume control on any
...
// docs/security/library-ingest-red/a-library-connection-status-request-past-the-50-request-limit-gets-429.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-rl-connection-'));
...
it('a library connection status request accepts under the 50 request limit', async () => {
  for (let i = 0; i < 5; i += 1) {
