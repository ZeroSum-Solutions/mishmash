// Baseline request-rate limit for POST /api/tools/library/search, closing
...
// docs/security/library-ingest-red/a-tools-library-search-request-past-the-50-request-limit-gets-429.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-rl-tools-search-'));
...
it('a tools library search request accepts under the 50 request limit', async () => {
  const { toolTokenRegistry } = await import('../src/tool-tokens.js');
  const grant = toolTokenRegistry.mint({ runId: 'w9-rl-search-accept', projectId: 'w9-rl-project' });
  for (let i = 0; i < 5; i += 1) {
...
it('a tools library search request past the 50 request limit gets 429', async () => {
  const { toolTokenRegistry } = await import('../src/tool-tokens.js');
  const grant = toolTokenRegistry.mint({ runId: 'w9-rl-search-reject', projectId: 'w9-rl-project' });
  let lastStatus = 0;
