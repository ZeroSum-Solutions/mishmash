...
describe('rewriteSkillAssetUrls', () => {
  it('rewrites ./assets/<file> img sources to the daemon route', () => {
    ...
    expect(rewriteSkillAssetUrls(html, 'skill-one')).toBe(
      `<img src='/api/skills/skill-one/assets/hero.png' alt='' />`,
    );
  });
  ...
  it('rewrites sibling skill asset references', () => {
    const html = `<img src='../skill-one/assets/hero.png' /><a href="../skill-two/assets/guide.pdf"></a>`;
    expect(rewriteSkillAssetUrls(html, 'foo')).toBe(
      `<img src='/api/skills/skill-one/assets/hero.png' /><a href="/api/skills/skill-two/assets/guide.pdf"></a>`,
    );
  });
