// Loopback gate for GET /api/library/assets/:id/figma. Per this file's own
...
// docs/security/library-ingest-red/get-library-assets-figma-sidecar-rejects-a-non-loopback-origin.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-lg-figma-'));
...
async function ingestAssetWithFigma(): Promise<string> {
...
it('GET library assets figma sidecar accepts a loopback request', async () => {
  const assetId = await ingestAssetWithFigma();
  const res = await fetch(`${baseUrl}/api/library/assets/${assetId}/figma`);
  expect(res.status).toBe(200);
});

it('GET library assets figma sidecar rejects a non-loopback origin', async () => {
  const assetId = await ingestAssetWithFigma();
  const origin = extOrigin();
  await mintToken(origin);
  const res = await fetch(`${baseUrl}/api/library/assets/${assetId}/figma`, { headers: { Origin: origin } });
