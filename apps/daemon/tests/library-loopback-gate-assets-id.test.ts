// Loopback gate for GET /api/library/assets/:id. Per this file's own
...
// docs/security/library-ingest-red/get-library-assets-by-id-rejects-a-non-loopback-origin.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-lg-assets-id-'));
...
it('GET library assets by id accepts a loopback request', async () => {
  const assetId = await ingestAsset();
  const res = await fetch(`${baseUrl}/api/library/assets/${assetId}`);
  expect(res.status).toBe(200);
});

it('GET library assets by id rejects a non-loopback origin', async () => {
  const assetId = await ingestAsset();
  const origin = extOrigin();
  await mintToken(origin);
  const res = await fetch(`${baseUrl}/api/library/assets/${assetId}`, { headers: { Origin: origin } });
