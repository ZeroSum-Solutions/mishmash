// Loopback gate for GET /api/library/assets/:id/raw. Per this file's own
...
// requireLocalDaemonRequest closes that (also the read-back half of the
// documented SSRF class: GET /api/library/assets/:id/raw). See
...
// docs/security/library-ingest-red/get-library-assets-raw-bytes-rejects-a-non-loopback-origin.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-lg-raw-'));
...
async function ingestAsset(): Promise<string> {
  const res = await fetch(`${baseUrl}/api/library/ingest`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text: 'hello world', kind: 'text' }),
  });
  const json = (await res.json()) as { asset: { id: string } };
  return json.asset.id;
}

it('GET library assets raw bytes accepts a loopback request', async () => {
  const assetId = await ingestAsset();
  const res = await fetch(`${baseUrl}/api/library/assets/${assetId}/raw`);
  expect(res.status).toBe(200);
});

it('GET library assets raw bytes rejects a non-loopback origin', async () => {
  const assetId = await ingestAsset();
  const origin = extOrigin();
  await mintToken(origin);
  const res = await fetch(`${baseUrl}/api/library/assets/${assetId}/raw`, { headers: { Origin: origin } });
