...
describe('agent runtime tool environment', () => {
  it('prefers explicit OD_NODE_BIN over the process executable', () => {
    expect(resolveOpenDesignNodeBin({
      env: { OD_NODE_BIN: 'C:\\MishMash\\resources\\open-design\\bin\\node.exe' },
      execPath: 'C:\\Users\\Ada\\AppData\\Roaming\\MishMash\\en\\hash\\MishMash.exe',
      platform: 'win32',
      resourceRoot: null,
    })).toBe('C:\\MishMash\\resources\\open-design\\bin\\node.exe');
  });
  ...
  it('resolves the bundled resource node before falling back to process.execPath', () => {
    expect(resolveOpenDesignNodeBin({
      ...
      execPath: 'C:\\Users\\Ada\\AppData\\Roaming\\MishMash\\en\\hash\\MishMash.exe',
      platform: 'win32',
      resourceRoot: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\MishMash\\resources\\open-design',
      exists: (candidate) => candidate.endsWith('\\resources\\open-design\\bin\\node.exe'),
    })).toBe('C:\\Users\\Ada\\AppData\\Local\\Programs\\MishMash\\resources\\open-design\\bin\\node.exe');
  });
