...
// /api/library/ingest (C9-6 P0 control): LIBRARY_UPLOAD_MAX_BYTES applies
// only to sourceKind==='manual-upload', and a URL-sourced fetch's own cap
// (MAX_REMOTE_BYTES) does not apply to dataUrl/text bodies -- the clipper
// class previously had no cap at all. See docs/security/daemon-threat-
...
// docs/security/library-ingest-red/a-clipper-ingest-payload-past-the-5000000-byte-limit-reject-with-413.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-rl-ingest-'));
...
it('a clipper ingest payload past the 5000000 byte limit: reject with 413', async () => {
  const origin = extOrigin();
  const token = await mintToken(origin);
