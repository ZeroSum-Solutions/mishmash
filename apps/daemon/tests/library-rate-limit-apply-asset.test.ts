// Baseline request-rate limit for POST /api/library/assets/:id/apply,
// closing the ground-facts gap "no request- or byte-volume control on any
...
// docs/security/library-ingest-red/a-library-assets-apply-request-past-the-50-request-limit-gets-429.txt.
...
beforeEach(async () => {
  dataDir = await mkdtemp(path.join(os.tmpdir(), 'od-library-rl-apply-asset-'));
...
it('a library assets apply request accepts under the 50 request limit', async () => {
  for (let i = 0; i < 5; i += 1) {
