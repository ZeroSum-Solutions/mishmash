...
// at `OD_WHATS_NEW_URL` (see whatsNewSourceUrl below — this fork has no
// built-in default document yet). The document carries an `id` that drives
// the home card's once-per-highlight behavior (see apps/web/src/lib/whats-new.ts).
// This service does not know about the running version — content identity is
// the only thing that matters. A missing, unreachable, or malformed document
 ...
 * MishMash does not host its own "what's new" document yet, so there is no
 * built-in default for any channel: falling back to the upstream Open Design
 * project's hosted feed would show a fork's users the wrong project's release
 * notes and depend on infrastructure this fork does not own. `OD_WHATS_NEW_URL`
 * is therefore the only source (used by e2e fixtures today, and by a future
 * MishMash-owned document once one is hosted) — set it to opt any channel in.
 */
...
export function createWhatsNewService({
  ...
  async function readWhatsNew(channel: string): Promise<WhatsNewReadResult> {
    ...
    if (sourceUrl == null) {
      // No OD_WHATS_NEW_URL configured for this channel: never show the card
