...
export function parseWrapperIframeSrc(html: string): string | null {
  ...
  if (!body) return null;
  const inner = (body[1] ?? '').replace(COMMENT_RE, '').replace(SCRIPT_RE, '').replace(STYLE_RE, '').trim();
...
export function resolveWrapperTarget(
  ...
  // root-absolute path is not resolvable inside a copied project tree.
  if (HAS_SCHEME_RE.test(bare) || bare.startsWith('//') || bare.startsWith('/')) return null;
...
function candidateIsFile(projectRoot: string, relPath: string): boolean {
  try {
    return statSync(path.join(projectRoot, ...relPath.split('/'))).isFile();
  } catch {
    return false;
  }
}
