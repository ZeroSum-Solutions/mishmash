...
function buildDesignManifest(entries, projectLabel) {
  ...
  return JSON.stringify({
    ...
    title: projectLabel || 'MishMash project',
    entryFile,
    sourceFiles: {
      all: files,
      html: htmlFiles,
...
function buildDesignHandoff(entries, projectLabel) {
  ...
  return `# ${projectLabel || 'MishMash project'} implementation handoff
...
- Keep generated product UI free of MishMash chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or MishMash chrome in the production UI.
