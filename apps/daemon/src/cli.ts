...
// external agents (hermes-agent, openclaw, etc.) can drive MishMash
...
function printRootHelp() {
  ...
  od plugin open-design-pr <folder>
      Push a community-catalog branch and open the MishMash PR form.
  ...
  od mcp [--daemon-url <url>]
      ...
      running MishMash daemon. Wire it into a coding agent
...
Runs Tavily-backed shallow research through the local MishMash daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The MishMash daemon itself is unaffected - it can be ' +
...
tool calls to a running MishMash daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local MishMash
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   MishMash daemon HTTP base URL. Resolution
...
Tools exposed:
  list_projects                  list every MishMash project
...
user has open in MishMash; get_artifact and get_file additionally
...
→ MCP server in the MishMash app. The daemon must be running locally
...
Register MishMash's stdio MCP server into a coding agent's own config.

Agents:
  ${AGENT_SLUGS.join(' ')}

Options:
  --uninstall, --remove   Remove the MishMash MCP server instead.
...
Wraps GitHub CLI auth for MishMash registry publishing. The token stays in gh.`);
    return;
  }
...
Shows the GitHub account gh will use for MishMash registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
...
Common options:
  --daemon-url <url>   MishMash daemon HTTP base (default OD_DAEMON_URL, OD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not MishMash.`);
...
// these so a code agent can drive MishMash end-to-end without
...
Imports a local project directory as an editable MishMash design system.
...
Imports a public GitHub repository as an editable MishMash design system.
...
async function runWhatsNew(args) {
  ...
  const data = await resp.json();
  if (flags.json) return process.stdout.write(JSON.stringify(data, null, 2) + '\n');
  console.log(`MishMash ${data?.version ?? 'unknown'}`);
