...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  function computeInstallPayload(): McpInstallPayload {
    ...
    // knows where the browser-facing MishMash studio is running.
  ...
  // External MCP server configuration. MishMash connects to these as a
...
function renderOAuthResultPage(opts: any) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to MishMash.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — MishMash</title>
