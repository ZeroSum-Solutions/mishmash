...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive MishMash metadata).');
  lines.push('');
  lines.push('Generated by `od plugin publish` — see `plugins/spec/SPEC.md` in the MishMash repository.');
  return lines.join('\n');
}
