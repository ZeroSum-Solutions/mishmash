...
function handleKimiEvent(obj: unknown, onEvent: StreamEventHandler): boolean {
  ...
  if (
    ...
    onEvent({
      ...
      // key. The one consistent signal found is that kimi's own Bash tool
      // wrapper appends a literal "Command failed with exit code: N." line
      // to `content` when the shell command's exit status is non-zero —
      // that text is generated by kimi's wrapper, not the command's own
      // output, so matching it here is safe. Other tools (Write, Edit,
      // etc.) surface raw OS/API error text (e.g. "EPERM: operation not
      // permitted...") with no consistent marker, so failures from those
      // tools still parse as isError:false — a known loop-guard blind spot
      // documented in TEAM-LANES.md's kimi caveats until kimi's
      // stream-json output gains a real structured status field.
      isError: /Command failed with exit code: \d+/.test(content),
    });
    return true;
  }
