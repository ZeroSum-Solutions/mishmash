...
// NM-33 (C1-10): kimi's `-p --output-format stream-json` `role:'tool'`
// records carry no structured error field at all -- the Bash-wrapper's own
// "Command failed with exit code: N." marker (still checked first, below)
// only covers Bash. Every other tool (Read/Write/Edit/…) surfaces the raw
// OS/API error text instead, with no consistent field to key off, so a
// content-based heuristic is the only signal available. Matches the
// well-known Node/POSIX errno mnemonics, a git-style `fatal:` line, a
// Python-style `OSError`/`Errno`, a `TimeoutError`, and a generic "exited
// with status N" tail -- deliberately NOT anchored to the start of the
// string, so a wrapper (`Error: <inner>`, `[tool-error] <inner>`, a
// trailing `\nexit status N`, a traceback) around one of these still
// matches on the inner text. Case-insensitive because kimi's own text
// casing is not guaranteed.
const KIMI_TOOL_FAILURE_RE =
  /\b(EPERM|ENOENT|EACCES|EEXIST|ENOTDIR|EISDIR|ENOSPC|EMFILE|EROFS|ETIMEDOUT|ECONNREFUSED|fatal|OSError|TimeoutError|Errno)\b|exit(?:ed)?\s+(?:with\s+)?status\s+\d+/i;

function kimiToolResultContentLooksLikeFailure(content: string): boolean {
  return (
    /Command failed with exit code: \d+/.test(content) ||
    KIMI_TOOL_FAILURE_RE.test(content)
  );
}
...
function handleKimiEvent(obj: unknown, onEvent: StreamEventHandler): boolean {
  ...
  if (obj.role === 'assistant' && Array.isArray(obj.tool_calls)) {
    // A single record can carry both a lead-in `content` string AND
    // `tool_calls` (verified live: kimi 0.27 print mode emits this when the
    // model narrates before acting, e.g. {role:'assistant',
    // content:"I'll attempt the write as requested.", tool_calls:[...]}).
    // Emit the text BEFORE the tool_use events instead of falling through
    // to the content-only branch below, which never runs once this branch
    // returns — that used to silently drop the narration text.
  ...
  if (
    ...
    const content = stringifyContent(obj.content);
    onEvent({
      type: 'tool_result',
      toolUseId: obj.tool_call_id.trim(),
      content,
      // kimi 0.27 print mode's `role:'tool'` records carry no structured
      // error field at all — verified empirically with two live failures:
      // a failed shell command (`cat /nonexistent-file-xyz`) and a Write to
      // a read-only path (`/System/...`), both produced a bare
      // {role,tool_call_id,content} shape with no `status`/`isError`/`ok`
      // key. kimi's own Bash tool wrapper appends a literal "Command failed
      // with exit code: N." line, matched first. Every other tool (Write,
      // Edit, …) surfaces raw OS/API error text instead (e.g. "EPERM:
      // operation not permitted…") — NM-33/C1-10:
      // kimiToolResultContentLooksLikeFailure generalizes the match to that
      // class of content so a non-Bash tool failure cannot silently parse
      // as isError:false (the loop guard's and the run's own silent-success
      // blind spot, until kimi's stream-json output gains a real structured
      // status field).
      isError: kimiToolResultContentLooksLikeFailure(content),
    });
    return true;
  }
