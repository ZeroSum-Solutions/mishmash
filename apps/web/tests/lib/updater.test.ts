...
describe('web updater model', () => {
    ...
    const model = deriveUpdaterModel(
      downloadedStatus({
        incoming: {
          arch: 'arm64',
          artifact: {
            name: 'MishMash Beta 1.2.3-beta.5.dmg',
            platformKey: 'macAppleSilicon',
            type: 'dmg',
            url: 'https://fixture.test/MishMash Beta 1.2.3-beta.5.dmg',
          },
          channel: 'beta',
    ...
    const model = deriveUpdaterModel(
      downloadedStatus({
        installResult: {
          ...
          path: '/tmp/open-design-updater/MishMash Beta.dmg',
        },
      }),
      { hostAvailable: true },
    );
  ...
  it('routes status, check, download, install, and quit requests through host helpers with flexible payloads', async () => {
    ...
    const install = vi.fn(async () => downloadedStatus({
      installResult: {
        dryRun: true,
        openedAt: '2026-05-19T00:00:00.000Z',
        path: status.downloadPath ?? '/tmp/open-design-updater/MishMash Beta.dmg',
      },
    }));
  ...
  it('routes native menu label sync and open-dialog subscriptions through the host', async () => {
    ...
    const labels = {
      ...
      restart: 'Restart to Update MishMash…',
    };
