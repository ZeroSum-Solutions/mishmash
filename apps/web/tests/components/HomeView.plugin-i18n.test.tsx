...
describe('HomeView plugin i18n', () => {
  ...
  it('routes the plugin card Use action as the active driver without hydrating the query', async () => {
    ...
    // The gallery left the Home view (studio-entrance restructure); plain
    // structure-only Use arrives through the Plugins-view handoff, the same
    // path EntryShell routes (`usePluginFromLibrary` → promptHandoff).
    const view = render(
