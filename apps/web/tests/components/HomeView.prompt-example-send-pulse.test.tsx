...
describe('use-with-query send pulse gating', () => {
  it('does not pulse Send when required inputs are still missing', async () => {
    ...
    // The gallery left the Home view (studio-entrance restructure); a
    // query-bearing Use arrives through the Plugins-view handoff, the same
    // path EntryShell routes. This plugin's required `topic` has no
    // default, so the composer lands on the inputs form with Send
    // disabled — pulsing it would point at a dead end.
    render(
      <I18nProvider initial="en">
        <div className="entry-main--scroll">
          <HomeView
            ...
            promptHandoff={createPluginUseHandoff(1, 'required-input-plugin', { action: 'use-with-query' })}
          />
        </div>
      </I18nProvider>,
    );
