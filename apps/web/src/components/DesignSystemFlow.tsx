    ...
    return (
      <div className="ds-setup-shell ds-setup-shell--center">
        <div className="ds-setup-center-card">
          <h1>MishMash will extract your design system.</h1>
  ...
  const sendProjectChatMessage = useCallback(
    async (
      ...
      if (config.mode !== 'daemon' || !config.agentId) {
        setChatError('Pick a local agent first, then ask MishMash to update this design system.');
        return;
      }
              ...
              <p>
                {generationActive
                  ? activeJob?.kind === 'token-contract-rebuild'
                    ? 'MishMash is preparing a token contract rebuild for review. The active contract stays unchanged until you accept it.'
                    : activeJob?.kind === 'revision'
                      ? 'MishMash is applying your feedback. You can keep reviewing while the updated draft is prepared.'
                      : 'MishMash is still working, but you can start giving feedback on the work so far.'
                  : 'MishMash is ready for review. Give feedback on the work so far, then publish when it is useful for future projects.'}
              </p>
              <label>
                <input
                  type="checkbox"
    ...
    <section className={`ds-workspace-activity-card is-${status}`}>
      <div className="ds-workspace-activity-head">
        ...
        <span>
          <strong>
            {status === 'running'
              ? 'MishMash is updating this system'
              : status === 'failed'
          ...
          <strong>
            {active
              ? job.kind === 'token-contract-rebuild'
                ? 'MishMash is rebuilding tokens'
                : job.kind === 'revision'
                  ? 'MishMash is revising'
                  : 'MishMash is still working'
              : job.status === 'failed'
...
function GitHubRepositoryAccessPanel({
  ...
  const methods: GitHubAccessMethod[] = [
    ...
    },
    {
      id: 'native-oauth',
      icon: 'link',
      title: 'MishMash account',
      badge: 'Coming soon',
      tone: 'muted',
      description: 'Native GitHub sign-in managed by MishMash; this build does not use a MishMash-managed GitHub token yet.',
    },
    {
      id: 'composio',
      icon: composioIcon,
...
function buildFallbackDesignMdFromState(state: SetupState): string {
  ...
  const overview =
    ...
    || state.notes.trim()
    || sourceNotes
    || 'Design system generated from source material supplied in MishMash.';
  return [
    '---',
...
function buildCreationAgentPrompt(
  ...
  return [
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that MishMash attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real MishMash design-system package.',
