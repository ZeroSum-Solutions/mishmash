...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# MishMash — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants MishMash wired into your workflow. MishMash is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported MishMash version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the MishMash app (packaged).');
  lines.push('');
  if (installInfo) {
