...
// TEMPORARY w9pre diagnostic — DELETE BEFORE THE PR.
// Measures the anchor chip against the InlineModelSwitcher safe region on each
// secondary entry page so the open-then-instantly-close cause is evidence, not a guess.
test('[DIAG] w9pre entry execution pill anchor geometry', async ({ page }) => {
  await routeDesignSystems(page);
  await gotoEntryHome(page);
  ...
  const geometry = () =>
    page.evaluate(() => {
      ...
      return {
        ...
        chip: rect('[data-testid="inline-model-switcher-chip"]'),
        scroll: rect('.entry-main--scroll'),
        topbar: rect('.entry-main__topbar'),
        popover: rect('[data-testid="inline-model-switcher-popover"]'),
      };
    });
  ...
  for (const destination of destinations) {
    ...
    console.log(`[diag-ec] ${destination.nav} BEFORE ${JSON.stringify(before)}`);
    console.log(`[diag-ec] ${destination.nav} AFTER  ${JSON.stringify(after)}`);
    await page.keyboard.press('Escape');
