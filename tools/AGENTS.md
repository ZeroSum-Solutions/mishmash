...
- Keep `tools-pack` focused on packaging/runtime control, release artifact preparation, and the packaged-updater acceptance harness. `apps/desktop` and `apps/packaged` (the updater product surface and launcher handoff) are absent from this fork; restoring them as apps is permitted (see root `AGENTS.md`), but their application logic still does not belong inside this tool.
