...
- Keep `tools-pack` focused on packaging/runtime control, release artifact preparation, and the packaged-updater acceptance harness. `apps/desktop` and `apps/packaged` (the updater product surface and launcher handoff) have been removed from this fork; do not restore that application logic in the tool.
