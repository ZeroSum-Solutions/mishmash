...
description: |
  ...
  MishMash browser-tab context, testing web apps, dogfooding MishMash
  previews, QA, bug hunts, or reviewing app quality. Prefer local MishMash
...
Use `agent-browser` for local MishMash preview validation: inspect rendered
...
For selected MishMash browser tabs and browser-use/browser-harness-style
...
5. If the selected MishMash context only provided a URL/title and no browser
...
Do not run MishMash's own daemon CLI as a browser automation tool. Commands
...
With the MishMash preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `MishMash`, current URL under `127.0.0.1:17573`,
visible MishMash UI text in the snapshot, and a screenshot at
