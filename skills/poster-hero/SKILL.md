...
example_desc: "A high-contrast launch poster with a QR code placeholder + gradient mesh + noise texture"
od:
  mode: prototype
  surface: web
  platform: desktop
...
- Top 30% left as whitespace + one large emoji or abstract geometric shape.
- Middle: main title as the visual center (text-8xl, font-black), one-line subtitle.
- Lower: an info card with 3-5 core points, each with an icon + short phrase.
- Bottom-right: brand / QR code (SVG placeholder).
- Use bold colors: gradient backgrounds (e.g. from-violet-500 via-fuchsia-500 to-indigo-500), white text + 1 contrasting highlight color.
