...
name: live-artifact
description: |
  Create refreshable, auditable MishMash artifacts backed by connector or local data.
...
Create a MishMash live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
5. **Register or update through daemon wrappers**
   - Use the MishMash daemon wrapper commands via `"$OD_NODE_BIN" "$OD_BIN"` instead of raw `curl`, bare `node`, or bare `od`:
