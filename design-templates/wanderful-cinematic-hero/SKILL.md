...
od:
  mode: prototype
  platform: desktop
  upstream: "https://github.com/pulkitxm/claude-directory/tree/main/hero-sections/wanderful-cinematic-hero"
  ...
  scenario: "marketing"
  example_prompt: "Build Wanderful — Cinematic Travel Hero Section as a self-contained responsive page in this template's own visual system. Follow the build spec in this skill exactly — palette, typography, section order, and motion are part of the identity. Ask only for the missing essentials first: brand name, real copy, and any imagery to swap in."
---

# Wanderful — Cinematic Travel Hero Section
...
```
<artifact identifier="wanderful-cinematic-hero" type="text/html" title="Wanderful — Cinematic Travel Hero Section">
<!doctype html>
<html>...</html>
</artifact>
```
...
# Wanderful — Cinematic Hero Section

## Overview

Build a full-viewport cinematic hero section for a travel brand called "Wanderful". A fixed, full-screen looping background video sits behind a fixed header, a centered two-line headline, and a bottom content block, all rendered over a black background. The video reacts to the cursor with a smooth GSAP-driven parallax drift, and the headline and bottom block fade in on mount.
...
- **Styling:** Tailwind CSS (`tailwindcss` `^3.4.15`) with `postcss` `^8.4.49` and `autoprefixer` `^10.4.20`
- **Animation:** GSAP (`gsap` `^3.12.5`) — used for the mouse-parallax `gsap.set` on each `requestAnimationFrame`
- **Icons:** Lucide (`lucide-react` `^0.468.0`) — the `Lock` icon
- **Fonts:** Inter (headings + root font family), Barlow (body), Instrument Serif, plus a custom display font "Dirtyline" loaded via `@font-face`
- **Notable techniques:** fixed full-screen video background, over-scaled wrapper for edge-safe parallax, lerped mouse parallax inside a `requestAnimationFrame` loop, custom "liquid glass" CSS utility, double-`requestAnimationFrame` mount transition
...
[`pulkitxm/claude-directory`](https://github.com/pulkitxm/claude-directory/tree/main/hero-sections/wanderful-cinematic-hero).
