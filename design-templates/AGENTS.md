...
1. Create a directory directly under `design-templates/` with a
   [`SKILL.md`](waitlist-page/SKILL.md) containing `name`,
...
## Vendored claude-directory templates

184 entries are vendored from the MIT-licensed
[`pulkitxm/claude-directory`](https://github.com/pulkitxm/claude-directory)
gallery: 41 hero sections, 98 landing pages, and 45 Lexington Themes
reproductions under the `lexington-` prefix. Each one carries a
`template.json` whose `vendored_from` field marks it as vendored, and a
`SKILL.md` whose body is the upstream build spec verbatim.

Do not hand-edit them. They are generated by
`scripts/import-claude-directory.ts`, which re-encodes upstream's imagery and
footage. Its temporary provider font requests must be followed by
`scripts/vendor-fonts.ts`, which replaces them with self-hosted catalogue
fonts. `pnpm guard` rejects both provider-loading references and a catalogue
that another local vendoring pass would change. A hand edit is lost on the
next import. The importer header documents the build prerequisites, the two
bake strategies, and the media budgets.

`pnpm guard` reads `template.json#vendored_from` to decide that JavaScript
under one of these directories is vendored bundler output rather than
project-owned source. A template directory without that marker gets no
exemption, so do not copy the field onto anything this repository authors.
