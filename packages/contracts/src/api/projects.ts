...
/**
 * F001 R8a -- one required section's provenance within a blended
 * multi-select (Addendum A.1: pick one direction wholesale, or mix sections
 * across directions -- "hero from #2, poem layout from #1"). `directionId`
 * is a free-form id matching whatever archetype-direction identifier the
 * advisor surfaced (e.g. `apps/daemon/src/design/site-archetypes.ts`'s
 * `ArchetypeDirection.id`) -- not typed against that daemon-side module
 * here, since contracts stays pure and cannot import it.
 */
export interface DesignAdvisorDirectionSelection {
  directionId: string;
  /** Archetype-required-section ids (F001 R3's `Archetype.requiredSections`) this direction's design supplies. */
  sections: string[];
}
...
export interface ProjectMetadata {
  ...
  // Design Library kit-starter provenance (Stream A): the catalog item's
  // `rel` path the project's files were copied from. Only ever set for
  // `licensed-source-review` / `own-code` allowed_use tiers — see
  // apps/daemon/src/routes/design-library.ts.
  duplicatedFromDesignLibraryRel?: string;
  // Prompt-only Design Library provenance. Unlike
  // duplicatedFromDesignLibraryRel, no licensed source bytes are copied into
  // the managed project; the first terminal prompt carries the bounded design
  // context and selected aspects instead.
  referencedFromDesignLibraryRel?: string;
  designLibraryReferenceSource?: string;
  designLibraryReferenceAspects?: string[];
  designLibraryReferenceHtmlSha256?: string;
  designLibraryReferenceDesignSha256?: string;
  /**
   * F001 R8a -- the conversational template advisor's blended multi-select
   * provenance: which named direction each required section's design came
   * from, so the choice stays inspectable later (F001 Addendum A.1: "one
   * project, blending the selections together" -- direction-level or
   * section-level picks share this one field). Optional and additive --
   * existing single-`skillId`/single-`templateId` project creation is
   * untouched. Populated via `POST /api/projects`'s existing `metadata`
   * field or `od project create --metadata-json` (both already merge
   * arbitrary `ProjectMetadata` server-side); no new endpoint or CLI flag
   * is needed for this half of R8a.
   */
  directionSelections?: DesignAdvisorDirectionSelection[];
